...
**ReAct amendment:** ADR-0021 changes ReAct's judgment trigger and verdict protocol. This ADR's Plan-step timing and `STEP_COMPLETION_PROGRESS` contract remain unchanged.
...
This decision amends ADR-0002, which permitted a nonempty no-tool response to complete a Step without separate judgment. Its protocol originally adapted ReAct's numbered evidence convention. ADR-0021 later changed ReAct's verdict schema, while the Plan-step judge retained its own `STEP_COMPLETION_PROGRESS` contract without a final `ANSWER`; the Executor model produces the Step handoff. The implementation contract is in [the Plan-step completion judgment spec](../../.scratch/plan-execute-completion-judgment/spec.md).
