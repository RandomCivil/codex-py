...
19. As an application owner, I want ReAct mode to return completed only when its final structured response has no further tool calls and explicitly declares `goal_satisfied: true`, so that an idle model response is not mistaken for goal completion.
20. As an application owner, I want ReAct to fail when the round budget is exhausted, the model/tool runtime fails unrecoverably, or its final response does not prove goal satisfaction, so that its terminal status is trustworthy.
...
- ReAct's terminal model response uses a strict structured contract containing a nonempty answer and `goal_satisfied: true`. It completes only when that declaration is valid and the response requests no more tools. ReAct does not create a Plan, invoke a Planner, create checkpoints, or resume an interrupted loop.
...
- Verify ReAct's multi-round correction after returned tool errors, concurrent ordered batch behavior, default and configured round budgets, successful structured goal assertion, rejection of an unproven goal, and failure on exhaustion.
