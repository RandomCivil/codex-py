...
13. As an Agent developer, I want a completed Step execution to allow no tool call when the model determines the Plan step needs none, so that already-complete or state-satisfiable work is not incorrectly failed.
14. As an Agent developer, I want the model's final answer to be a strict JSON Schema completion receipt declaring completion and a concise result, so that completion is machine-validatable rather than inferred from prose.
...
16. As an Agent developer, I want a final model response without a valid strict JSON Schema completion receipt to fail the Step execution, so that unsupported conversational exits cannot be recorded as success.
17. As an Agent developer, I want an MCP tool error returned to the model as error context, so that it can choose a corrective next action without the Executor triggering replanning.
18. As an Agent developer, I want Atom `exec` with a nonzero exit code returned to the model as error context, so that it can choose a corrective next action without that failure counting as a successful tool call.
...
- The model instruction identifies the goal, selected Plan step, its intent and completion criterion, and the complete serializable Agent state; it may use MCP tools when needed. After the MCP loop ends, a separate binding requires a strict JSON Schema response containing `completed: true`, `completion_criterion_met: true`, and a non-empty string `result` that explains the work. This isolates structured completion from MCP tools whose schemas are not necessarily strict-output compatible.
- A completed execution may have no ToolNode invocation, provided the valid JSON completion receipt attests that the completion criterion was met. A malformed/extra-field JSON response, false completion flag, missing/non-text result, model exception, or exhausted fifty-round default budget yields `StepExecution(status="failed", error=...)`. If tools were invoked, at least one must succeed.
- An MCP tool error is returned to the model as an error `ToolMessage`, including when the ToolNode itself raises. A result from Atom's `exec` tool whose structured exit code is nonzero is likewise marked as an error `ToolMessage`. The Executor does not replan or retry independently; the model may choose its next action within the remaining round budget. Neither case counts as a successful tool invocation.
...
- Verify correct Plan-step resolution, full Agent-state context, successful tool and no-tool executions, and a final result that satisfies the strict JSON Schema completion contract.
- Verify failures for an unknown revision/step, malformed or invalid JSON completion response, model failure, and exhausted tool-round budget. Verify that MCP errors and Atom `exec` nonzero exit codes are returned to the model for recovery, without mutation of the input Agent state or Plan.
...
- Executor-owned retries, recovery, fallback tools, or replanning after any MCP error, nonzero Atom `exec` exit code, or failed Step execution. The model may select a corrective next tool call after receiving an error `ToolMessage`.
