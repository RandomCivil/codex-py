...
11. As an application owner, I want at most ten tool rounds per Step execution by default, so that a malformed or indecisive model cannot produce unbounded side effects.
...
14. As an Agent developer, I want the model's final answer to be strict JSON declaring completion and a concise result, so that completion is machine-validatable rather than inferred from prose.
...
16. As an Agent developer, I want a final model response without a required tool call or valid completion JSON to fail the Step execution, so that unsupported conversational exits cannot be recorded as success.
...
- The model instruction identifies the goal, selected Plan step, its intent and completion criterion, the complete serializable Agent state, the requirement to use MCP tools, and the final response contract. Its only successful final response is exactly a JSON object with `completed: true` and a non-empty string `result` that relates the work to the completion criterion.
- A completed execution requires at least one successful ToolNode invocation. A no-tool response, malformed/extra-field final JSON, false completion flag, missing/non-text result, model exception, or exhausted ten-round default budget yields `StepExecution(status="failed", error=...)`.
...
- Verify correct Plan-step resolution, full Agent-state context, a successful at-least-one-tool execution, and a final result that satisfies the strict JSON contract.
- Verify failures for an unknown revision/step, no tool call, malformed or invalid final JSON, model failure, MCP error, Atom `exec` nonzero exit code, and exhausted tool-round budget. Verify those outcomes do not mutate the input Agent state or Plan.
