...
- A completed execution may have no ToolNode invocation, provided the valid completion-tool receipt attests that the completion criterion was met. A malformed/extra-field completion-tool call, false completion flag, missing/non-text result, model exception, or exhausted fifty-round default budget yields `StepExecution(status="failed", error=...)`. If tools were invoked, at least one must succeed.
...
- Verify correct Plan-step resolution, full Agent-state context, successful tool and no-tool executions, and a final result that satisfies the forced strict completion-tool contract.
- Verify failures for an unknown revision/step, malformed or invalid completion-tool call, model failure, MCP error, Atom `exec` nonzero exit code, and exhausted tool-round budget. Verify those outcomes do not mutate the input Agent state or Plan.
